...
export function AddDonationPage({ onNavigate }: AddDonationPageProps) {
  ...
  const fetchData = async () => {
    ...
    if (profile) {
      ...
      setMyLeader(found);
      if (found) {
        setForm(f => ({ ...f, leaderId: found.id }));
      }
    }

    if (loadedProjects.length > 0) {
      setForm(f => ({ ...f, projectId: loadedProjects[0].id }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: donor, error: donorErr } = await supabase
      .from('donors')
      .insert({ nom: form.donorName.trim() || 'Anonyme' })
      .select()
      .single();

    if (donorErr) {
      setError('Erreur lors de l\'enregistrement du donateur.');
      setLoading(false);
      return;
    }
      ...
      {success && (
        <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
          ...
          <div>
            ...
            <button onClick={() => onNavigate('history')} className="text-sm text-emerald-600 hover:underline">
              Voir l'historique →
            </button>
          </div>
        </div>
              ...
              {projects.map(p => (
                ...
                >
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                    form.projectId === p.id ? 'border-emerald-500 bg-emerald-500' : 'border-gray-300'
                  }`}>
          ...
          {isAdmin && (
            <div>
              ...
              >
                ...
                {leaders.map(l => (
                  <option key={l.id} value={l.id}>{l.nom_affichage}</option>
                ))}
              </select>
            </div>
          )}
          ...
          {!isAdmin && myLeader && (
            <div className="flex items-center gap-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
              ...
              <p className="text-sm text-emerald-700">
                Ce don sera attribué à <span className="font-semibold">{myLeader.nom_affichage}</span>
              </p>
            </div>
          )}
