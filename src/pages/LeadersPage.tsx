...
export function LeadersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const data = {
      nom_affichage: form.nom_affichage,
      slug: form.slug || slugify(form.nom_affichage),
      user_id: form.userId || null,
    };
    if (editingId) {
      await supabase.from('leaders').update(data).eq('id', editingId);
    } else {
      await supabase.from('leaders').insert(data);
    }
  ...
  const copyLink = (slug: string, id: string) => {
    const link = `${window.location.origin}/#/donate?ref=${slug}`;
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  setForm(f => ({ ...f, nom_affichage: v, slug: f.slug || slugify(v) }));
                }}
                required
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-emerald-400/40 focus-within:border-emerald-400 transition-all">
                <span className="px-3 py-3 bg-gray-50 text-gray-400 text-sm border-r border-gray-200">/donate?ref=</span>
                <input
                  type="text"
                  value={form.slug}
                  onChange={e => setForm(f => ({ ...f, slug: e.target.value }))}
                  className="flex-1 px-3 py-3 text-sm focus:outline-none"
                  placeholder="frere-ahmed"
                />
              </div>
            </div>
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-400/40 focus:border-emerald-400 bg-white text-sm"
              >
                <option value="">— Aucun compte lié —</option>
                {profiles.map(p => (
                  <option key={p.id} value={p.id}>{p.nom || p.email}</option>
                ))}
              </select>
            </div>
            ...
            <div key={l.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-all hover:-translate-y-0.5">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleEdit(l)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors">
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(l.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <div className="bg-gray-50 rounded-xl p-3">
                  ...
                  <p className="font-bold text-gray-900 text-sm mt-0.5">{formatAmount(stats.total)}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
              ...
              <button
                ...
                className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-xl text-sm font-medium transition-all"
              >
