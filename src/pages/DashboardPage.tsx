...
export function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  const fetchAll = async () => {
    ...
    const pStats: ProjectStats[] = projects.map(p => ({
      project: p,
      total: donations.filter(d => d.project_id === p.id).reduce((s, d) => s + d.montant, 0),
    }));
    setProjectStats(pStats);

    const lStats: LeaderStats[] = leaders.map(l => {
      const lDonations = donations.filter(d => d.leader_id === l.id);
      return {
        leader: l,
        total: lDonations.reduce((s, d) => s + d.montant, 0),
        count: lDonations.length,
      };
    }).sort((a, b) => b.total - a.total);
    setLeaderStats(lStats);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={leaderStats.filter(l => l.count > 0).length}
          icon={TrendingUp}
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-5">
              ...
              <div key={project.id}>
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{formatAmount(total)} collectés</p>
                  </div>
                  {project.objectif > 0 && (
                    <p className="text-xs text-gray-400">objectif {formatAmount(project.objectif)}</p>
                  )}
                </div>
            ...
            {projectStats.length === 0 && (
              <p className="text-gray-400 text-sm text-center py-4">Aucun projet disponible</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-3">
            {leaderStats.map(({ leader, total, count }, i) => (
              <div key={leader.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
            ...
            {leaderStats.length === 0 && (
              <p className="text-gray-400 text-sm text-center py-4">Aucun responsable disponible</p>
            )}
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {recentDonations.map(d => (
                  <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 text-sm font-bold text-emerald-600">{formatAmount(d.montant)}</td>
