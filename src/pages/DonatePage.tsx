...
export function DonatePage({ leaderSlug }: DonatePageProps) {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      const secret = await createPaymentIntent(finalAmount);
  ...
  const handleCashSubmit = async (e: React.FormEvent) => {
    ...
    const { data: donor, error: donorErr } = await supabase
      .from('donors')
      .insert({ nom: donorName.trim() || 'Anonyme' })
      .select()
      .single();
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950">
      ...
      <div className="relative max-w-lg mx-auto px-4 py-12">
                  ...
                  {projects.map(p => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition-all ${
                        selectedProject === p.id ? 'border-emerald-500 bg-emerald-50' : 'border-gray-100 hover:border-gray-200 bg-gray-50'
                      }`}
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        selectedProject === p.id ? 'border-emerald-500 bg-emerald-500' : 'border-gray-300'
                      }`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${
                      paymentMethod === 'card' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300 bg-gray-50'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${
                      paymentMethod === 'cash' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300 bg-gray-50'
                    }`}
                  >
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      ...
                      {finalAmount > 0 ? `Payer ${finalAmount.toLocaleString('fr-FR')} € par carte` : 'Payer par carte'}
                    </>
                  )}
                </button>
              ) : (
            ...
            <form onSubmit={handleStripePayment} className="space-y-5">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    <Lock className="w-4 h-4" />
                    Confirmer le don de {finalAmount.toLocaleString('fr-FR')} €
                  </>
                )}
              </button>
              ...
              <button
                type="button"
                onClick={() => { setClientSecret(''); cardMountedRef.current = false; elementsRef.current = null; }}
        ...
        <p className="text-center text-emerald-400/50 text-xs mt-6">
          &copy; {new Date().getFullYear()} JamaaAmanah
        </p>
      </div>
    </div>
  );
