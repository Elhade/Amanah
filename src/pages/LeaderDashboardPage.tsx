...
export function LeaderDashboardPage({ onNavigate }: LeaderDashboardPageProps) {
  ...
  const fetchData = async () => {
    ...
    const myDons = leader ? allDonations.filter(d => d.leader_id === leader.id) : [];
    setMyDonations(myDons);

    const leaders = (allLeadersRes.data || []) as Leader[];
    const rankData: LeaderRanking[] = leaders.map(l => {
  ...
  const donationLink = myLeader
    ? `${window.location.origin}/#/donate?ref=${myLeader.slug}`
    : '';
  ...
  if (profile?.role === 'super_admin' && !myLeader) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">Mon Espace Responsable</h1>
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
          <Trophy className="w-12 h-12 text-amber-400 mx-auto mb-3" />
          <p className="text-gray-600">Vous êtes Super Admin. Pour voir un espace responsable, créez un responsable lié à votre compte.</p>
        </div>
            ...
            {ranking.map(({ leader, total, count }, i) => (
              <div key={leader.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                ...
                <div className="flex-1">
                  <p className="font-medium text-gray-800 text-sm">{leader.nom_affichage}</p>
                  <p className="text-xs text-gray-400">{count} don{count > 1 ? 's' : ''}</p>
                </div>
                <p className="font-bold text-gray-900 text-sm">{formatAmount(total)}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard title="Total collecté" value={formatAmount(myTotal)} icon={DollarSign} iconColor="text-emerald-600" iconBg="bg-emerald-50" />
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          {myDonations.length === 0 ? (
            <div className="text-center py-6">
              ...
              <button onClick={() => onNavigate('add-donation')} className="mt-2 text-emerald-600 text-sm font-medium hover:text-emerald-700">
                Ajouter un don →
              </button>
            </div>
          ) : (
              ...
              {myDonations.slice(0, 6).map(d => (
                <div key={d.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-emerald-600 text-sm">{formatAmount(d.montant)}</span>
            ...
            {ranking.map(({ leader, total, count }, i) => {
              ...
              return (
                <div key={leader.id} className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isMe ? 'bg-emerald-50 border-2 border-emerald-200' : 'bg-gray-50'}`}>
                  ...
                  <p className="font-bold text-sm text-gray-900">{formatAmount(total)}</p>
                </div>
              );
            })}
          </div>
